...
EVIDENCE_COMMIT = '3c2b26e91ea8f2f86683d671bf09b09ea0b42787'
REPORT_URL = f'https://raw.githubusercontent.com/andremaio/nexus-kaggle-bridge/{EVIDENCE_COMMIT}/reports/qwen3-4b-probe-v1.json'
DECISION_URL = f'https://raw.githubusercontent.com/andremaio/nexus-kaggle-bridge/{EVIDENCE_COMMIT}/training/decision_holdout_v1.jsonl'
...
def generate(model, tokenizer, cases: list[dict], label: str) -> dict[str, str | None]:
    import torch
    ...
    for index, case in enumerate(cases, 1):
        ...
        print(f'{label} {index}/{len(cases)} {case_id}: label={decision!r}', flush=True)
    return decisions
...
def main() -> None:
    ...
    cand_decisions = generate(model, tokenizer, cases, 'QWEN4B_DECISION')
    cleanup(model)
    ...
    v4_decisions = generate(v4_model, v4_tokenizer, cases, 'V4_DECISION')
    ...
    report = {
        'schema': 'nexus.training.decision-probe.qwen3-4b.v2',
        'model_id': MODEL_ID,
    ...
    Path(OUT / 'qwen3-4b-decision-probe-v2.json').write_text(
...
if __name__ == '__main__':
    try:
        ...
        failure={
            'schema':'nexus.training.decision-probe.qwen3-4b.failure.v2',
        ...
        Path(OUT/'qwen3-4b-decision-probe-failure-v2.json').write_text(json.dumps(failure,ensure_ascii=False,sort_keys=True,indent=2)+'\n')
