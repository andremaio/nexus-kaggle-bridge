...
def fetch_jsonl(name: str, commit: str, blob: str) -> list[dict]:
    ...
    req=urllib.request.Request(api,headers={'Accept':'application/vnd.github+json','User-Agent':'nexus-qwen4b-v2-eval'})
...
def main() -> None:
    ...
    report={
        'schema':'nexus.eval.qwen3-4b-instruct2507-lora.v2',
    ...
    dump(OUT/'qwen3-4b-instruct2507-lora-v2-eval.json',report)
    print('NEXUS_QWEN4B_V2_EVAL_COMPLETE',flush=True); print(json.dumps(report,ensure_ascii=False,sort_keys=True),flush=True)


if __name__=='__main__':
    try: main()
    except Exception as exc:
        dump(OUT/'qwen3-4b-instruct2507-lora-v2-eval-failure.json',{
            'schema':'nexus.eval.qwen3-4b-instruct2507-lora.v2.failure','error_type':type(exc).__name__,'error':str(exc),'traceback':traceback.format_exc()[-20000:],
